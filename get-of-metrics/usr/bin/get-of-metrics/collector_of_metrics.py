...
class GetMetrics:
    ...
    def execute(self):
        ...
        if self.ssh.get_transport().is_active():
            ...
            REGISTRY.register(family_of_metrics.RxPacket(self.alias_name))
            REGISTRY.register(family_of_metrics.TxPacket(self.alias_name))
            REGISTRY.register(family_of_metrics.RxBytes(self.alias_name))
            REGISTRY.register(family_of_metrics.TxBytes(self.alias_name))
            REGISTRY.register(family_of_metrics.RxErrors(self.alias_name))
            REGISTRY.register(family_of_metrics.TxErrors(self.alias_name))
            REGISTRY.register(family_of_metrics.RxDrops(self.alias_name))
            REGISTRY.register(family_of_metrics.TxDrops(self.alias_name))
            REGISTRY.register(family_of_metrics.RxFrameErrors(self.alias_name))
            REGISTRY.register(family_of_metrics.RxOverErrors(self.alias_name))
            REGISTRY.register(family_of_metrics.RxCrcErrors(self.alias_name))
            REGISTRY.register(family_of_metrics.Collisions(self.alias_name))
            while True:
                data = self.collect(1)
